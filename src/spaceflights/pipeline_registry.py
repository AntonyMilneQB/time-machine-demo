...
def register_pipelines() -> Dict[str, Pipeline]:
    ...
    fps = [pipeline(data_science_pipeline, inputs={"model_input_table"}, namespace=f"pipeline_{i}") for i in range(20)]

    return {
        "__default__": unfiltered_pipeline + filtered_pipeline + Pipeline(fps[:20]),
